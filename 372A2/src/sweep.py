...
def main(out="results.csv"):
    ...
    with open(out, "w", newline="") as fh:
        ...
        w.writerow(["size_label", "size_bytes", "loss", "run", "seconds"])
        for label in SIZES:
            for loss in LOSSES:
                for run in range(1, RUNS + 1):
                    secs = run_once(paths[label], loss)
                    received = "recieved_" + os.path.basename(paths[label])


                    if os.path.exists(received): os.remove(received)

                    w.writerow([label, SIZES[label], loss, run, f"{secs:.6f}"])
                    fh.flush()


                    print(f"{label} loss={loss} run={run} {secs:.3f}s")
